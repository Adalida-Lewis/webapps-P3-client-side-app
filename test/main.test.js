...
QUnit.test('TEST submitFirst', assert => {
    assert.equal(submitFirst(), -1, 'You have entered an invalid number')
    assert.equal(submitFirst(), 11, 'You have entered an invalid number')
    assert.equal(submitFirst(), 10, 'Valid starting input. The game has started.')
    assert.equal(submitFirst(), 1, 'Valid starting input. The game has started.')
    assert.equal(submitFirst(), 0, 'You have entered an invalid number')
})
...
window.addEventListener('load', () => {
    const appURL = '../mini-game.html' // reach out to the html for the app (js-gui)
